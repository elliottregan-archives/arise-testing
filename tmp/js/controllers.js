mediaBrowser.controller('mediaBrowserController', function($scope) {
  var init;
  init = function() {
    console.log("index!");
    $scope.mediaLibrary = [
    ...
    $scope.search = function(item) {
      var conditions;
      conditions = item.title.indexOf($scope.searchParameter) !== -1 && (item.category.indexOf($scope.mediaFilter) !== -1 || $scope.mediaFilter === void 0);
      if (conditions) {
        return true;
      }
      return false;
    ...
    return $scope.toggleFilterDropdown = function(filterCategoryIndex) {
      ...
      if (filterCategoryIndex !== void 0) {
        $scope.filterCategories = $scope.filterOptions[filterCategoryIndex].options;
        return $scope.filterName = $scope.filterOptions[filterCategoryIndex].name;
      }
    };
  };
  return init();
